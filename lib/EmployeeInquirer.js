...
function addEmployee() {
  database.getAllEmployees()
    .then(employees => {
      const managerNames = employees.map(e => `${e["Given Name"]} ${e.Surname}`)
      ...
      database.getAllRoles()
        .then(roles => {
          ...
          ]).then(({givenName, surname, roleName, managerName}) => {
            database.addEmployee(givenName, surname, roleName, managerName)
              .then(result => promptMainMenu())
          })
        })
    })
...
function removeEmployee() {
  database.getNonmanagerEmployees()
  .then(employees => {
    const employeeNames = employees.map(e => `${e["Given Name"]} ${e.Surname}`)
    employeeNames.push("(cancel)")
    ...
    inquirer.prompt([
      ...
      if ("(cancel)" === employeeName) {
        ...
        database.removeEmployee(employeeName)
          .then(result => promptMainMenu())
      }
    })
  })
      ...
      database.getAllRoles()
        .then(roles => {
          ...
          inquirer.prompt([
            ...
            database.setEmployeeRole(employeeName, roleName)
              .then(result => promptMainMenu())
          })
        })
    })
...
function setEmployeeManager() {
  database.getAllEmployees()
  .then(employees => {
    const
      employeeNames = employees.map(e => `${e["Given Name"]} ${e.Surname}`),
      managerNames = employeeNames.slice()
    ...
    inquirer.prompt([
      ...
      if (employeeName === managerName) {
        ...
        database.setEmployeeManager(employeeName, managerName)
          .then(result => promptMainMenu())
      }
    })
  })
